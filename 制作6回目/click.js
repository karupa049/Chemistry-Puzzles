        ...
        if (!menuOpen) {
            ...
            if (mouseX < gameAreaWidth && draggingElement) {
                ...
                if (mouseButton === LEFT || mouseButton === RIGHT) { 
                    ...
                    // クリック配置のため、draggingElementは解除しない（連続配置を可能にする）
                    return;
                }
            }
        }
...
function findCombinationHints() {
    combinationHints = [];
    let unreacted = placedGameElements.filter(el => !el.reacted && !el.isDead());
