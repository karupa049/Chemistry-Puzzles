...
function findCombinationHints() {
    ...
    // 1. ドラッグ中でなければヒントを表示しない
    if (!draggingElement) return;

    // 2. 配置済みの元素と、ドラッグ中の元素の組み合わせをチェック
    for (let placedEl of placedGameElements) {
        // 既に反応済みのものや死亡しているものは無視
        if (placedEl.reacted || placedEl.isDead()) continue;

        // ドラッグ中の位置(mouseX, mouseY)と配置済み元素の距離をチェック
        let d = dist(mouseX, mouseY, placedEl.x, placedEl.y);

        // 反応距離内にある場合のみレシピを検索
        if (d < REACTION_DISTANCE) {
            
            // 全レシピを走査して「ドラッグ中の元素」と「近くの配置済み元素」を含むレシピを探す
            for (let recipe of recipes) {
                let ingredients = recipe.ingredients;
                
                // レシピに「ドラッグ中の元素」と「配置済みの元素」の両方が含まれているか確認
                // (例: H2Oなら、HとOが含まれる。H同士でもOK)
                let hasDragging = ingredients[draggingElement.name] > 0;
                let hasPlaced = ingredients[placedEl.name] > 0;

                if (hasDragging && hasPlaced) {
                    // ヒント情報を追加
                    combinationHints.push({
                        x: placedEl.x,
                        y: placedEl.y - 30, // 元素の少し上に表示
                        name: recipe.name,  // 完成する化合物名 (例: H2O)
                        color: recipe.color
                    });
                    
                    // 1つの元素に対して複数のレシピ候補がある場合も考慮しつつ、
                    // ここでは最初のマッチのみ表示してbreakするか、すべて表示するか選べますが
                    // 見やすさのため最も優先度の高い1つを表示してループを抜けます
                    break; 
                }
            }
        }
