...
// 1. 状態管理変数
// ===================================
let gameState = 'start'; // ゲームの状態: 'start', 'stage', 'gameover', 'editor', 'win'
...
let gameObstacles = [];   // ゲームプレイ中の「防衛壁（Barricade）」 (HPを持つ障害物)
let lastSpawnTime = 0;        // 最後に敵を生成した時間
const SPAWN_INTERVAL = 1000;  // 敵を生成する間隔 (ms)
let enemiesSpawned = 0;       // 新規: 出現させた敵の総数
let enemiesDefeated = 0;      // 新規: 撃破した敵の総数
const TOTAL_ENEMIES_TO_SPAWN = 100; // 新規: ステージで出現させる敵の総数
...
function initializeStage() {
    ...
    currentEnergy = 100;
    enemiesSpawned = 0; // カウンターリセット
    enemiesDefeated = 0; // カウンターリセット
    loop();
}
