...
const SPAWN_INTERVAL = 500; // 敵を生成する間隔 (ms)
...
const PATH_NODE_SIZE = 20; // ノードの当たり判定に使用
...
// ゲーム開始時および初期化時にデフォルト経路を設定
function setDefaultPathNodes() {
    pathNodes = [];
    // 画面の幅と高さを利用して経路を設定
    pathNodes.push({ x: 50, y: height / 4, radius: PATH_NODE_SIZE, isStart: true });
    pathNodes.push({ x: gameAreaWidth / 2, y: height / 4, radius: PATH_NODE_SIZE, isStart: false });
    pathNodes.push({ x: gameAreaWidth / 2, y: height * 3 / 4, radius: PATH_NODE_SIZE, isStart: false });
    pathNodes.push({ x: gameAreaWidth - 50, y: height * 3 / 4, radius: PATH_NODE_SIZE, isStart: false });
}
    ...
    if (gameState === 'stage') {
        ...
        if (!menuOpen) {
            for (let el of periodicTableElements) {
                if (el.isMouseOver()) {
                    // ★エネルギーチェック
                    let cost = elementCosts[el.name] || 0;
                    if (currentEnergy >= cost) {
                        ...
                        console.log(`エネルギー不足！${el.name}の配置に必要なコスト: ${cost}`);
                    }
                    break;
                }
            }
...
function initializeStage() {
    ...
    // ★修正: 経路がなぜか空の場合は、ここで再生成する（これにより即スタート時の不具合を防止）
    if (!pathNodes || pathNodes.length === 0) {
        setDefaultPathNodes();
        console.log("経路データを再生成しました");
    }
...
class Enemy {
    ...
    followPath() {
        ...
        // ★修正: deltaTimeが不安定な場合(0やNaN)のフォールバックとして16msを使用
        let dt = (deltaTime || 16) / 1000; 
        ...
        if (currentDist <= moveAmount || currentDist < this.nodeThreshold) {
            this.x = targetNode.x;
            this.y = targetNode.y;
        ...
        if (mouseButton === LEFT) {
            ...
            if (editorState === 'path') {
                for (let i = pathNodes.length - 1; i >= 0; i--) {
                    // isStartノード（最初のノード）は削除不可とする
                    if (!pathNodes[i].isStart && dist(mouseX, mouseY, pathNodes[i].x, pathNodes[i].y) < pathNodes[i].radius) {
...
function drawPathNodes() {
    ...
    // ★修正: パスの描画（透過ロジックを廃止し、常にハッキリ表示する）
    stroke(255, 0, 0, 200); // 常に高い不透明度
    strokeWeight(3);
    noFill();
    beginShape();
    for (let node of pathNodes) {
    ...
    for (let i = 0; i < pathNodes.length; i++) {
        ...
        let fillColor;
        if (i === 0) fillColor = color(0, 255, 0, 255); // スタート (緑)
        else if (i === pathNodes.length - 1) fillColor = color(0, 0, 255, 255); // エンド/ベース付近 (青)
        else fillColor = color(255, 255, 0, 255); // 中間ノード (黄色)
