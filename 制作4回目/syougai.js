...
let gameObstacles = [];   // ★追加: ゲームプレイ中の「防衛壁（バリケード）」
...
// --- ステージエディター関連 ---
let editorState = 'path'; // エディターのサブ状態: 'path', 'obstacle'
let obstacles = [];       // 障害物の配列（エディター用データ）
...
function draw() {
    if (gameState === 'start') {
        ...
        // ★変更: ゲーム中は「エディタ用obstacles」ではなく「HPのあるBarricade」を描画
        for (let bar of gameObstacles) {
            bar.draw();
        }

        drawPathNodes(); // 経路表示（プレイ中は薄く表示してもよい）
...
function initializeStage() {
    ...
    // ★追加: エディタで作った障害物データを、ゲーム用のバリケード(Barricade)に変換
    gameObstacles = [];
...
class Enemy {
    constructor(x, y, path) {
        ...
        this.isBlocked = false; // 壁に阻まれているか
    }
