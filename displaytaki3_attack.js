...
    
    // 2. 敵の移動 (更新)
    // 敵を逆順に処理することで、配列から削除してもインデックスが狂わないようにする
    for (let i = enemies.length - 1; i >= 0; i--) {
        let e = enemies[i];
        e.move1(gameAreaWidth, height); 
        
        // 敵が画面外に出た、またはHPが0になったら削除
        if (e.x < 0 || e.y < 0 || e.x > width || e.y > height || e.isDead()) {
...
        bullet.update(); // 弾丸を移動させる

        // 敵との衝突判定
        let hitEnemy = bullet.hitscan(enemies);
        if (hitEnemy) {
            hitEnemy.takeDamage(bullet.damage);
            bullets.splice(i, 1); // 弾丸はヒットしたら消滅
            continue;
        }

        // 弾丸が画面外に出たら削除 (画面外に出た後も直進するように修正したため、このチェックが重要)
...
        // ★ 追加 ★ タワーの攻撃能力
        this.range = 150; // 攻撃範囲
        this.attackDamage = 10; // 攻撃力
        this.fireRate = 60; // 発射速度 (フレーム数, 60で1秒に1回)
...
        // push();
        // noFill();
        // stroke(255, 0, 0, 100); // 半透明の赤
        // ellipse(this.x, this.y, this.range * 2, this.range * 2);
        // pop();
    }

    // ★ 追加 ★ ターゲットを見つけて攻撃するメソッド
    findTargetAndAttack(enemiesArray, bulletsArray) {
        // 攻撃クールダウン中なら何もしない
        if (millis() - this.lastAttackTime < this.fireRate * (1000 / 60)) { // millis()はms単位
            return;
        }
...
        let hpBarHeight = 5;
        let hpRatio = this.health / 100; // 最大HPを100とする
        fill(0, 255, 0); // 緑色のHPバー
        noStroke();
...
        this.x = startX;
        this.y = startY;
        this.target = targetEnemy; // 追跡する敵
        this.damage = damage;
        this.color = color;
        this.speed = 5; // 弾速
        this.size = 10; // 弾のサイズ

        // ★ 追加 ★ ターゲットを失った後の移動方向を保存するためのプロパティ
        this.vx = 0;
        this.vy = 0;
        this.hasTarget = true; // ターゲットが存在するかどうか
    }

    update() {
        if (this.hasTarget) {
            // ターゲットが死んだ、または何らかの原因で消滅した場合
            if (!this.target || this.target.isDead()) {
                // ターゲットを失った！直進モードに切り替え
                this.hasTarget = false;
                
                // 最後に敵がいた方向のベクトルを計算し、保存
...
        }
        
        // ターゲットを失った（hasTarget: false）後の直進処理
        this.x += this.vx;
        this.y += this.vy;
    }
