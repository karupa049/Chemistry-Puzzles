...
function draw() {
    if (gameState === 'start') {
        ...
        drawEnergyBar();
        drawDefeatedCount();
        drawUnitInfoPanel(); // ★追加: ユニット詳細パネルの描画
        drawMenuOverlay();
...
function mousePressed() {
    ...
    if (gameState === 'stage') {
        ...
        if (!menuOpen) {
            // ★修正: ゲームエリア内の既存ユニットクリック判定（ステータス表示更新）
            if (mouseX < gameAreaWidth) {
                for(let el of placedGameElements) {
                    if (dist(mouseX, mouseY, el.x, el.y) < el.size / 2) {
                        // 既存ユニットをクリックしたらその情報をパネルにセット
                        updateInfoPanelFromUnit(el);
                        // ここでreturnすると配置ができなくなるが、
                        // 既存ユニットの上には配置できない仕様(overlapチェック)があるので問題なし
                        // ただし右クリック等の処理は続行したいのでreturnはしない
                    }
                }
            }
            ...
            for (let el of periodicTableElements) {
                if (el.isMouseOver()) {
                    ...
                    // ★追加: 周期表からドラッグ開始した時点で、その元素の基本情報をパネルにセット
...
function updateInfoPanelFromUnit(unitObj) {
    ...
    lastSelectedUnit = {
        ...
        attack: floor(unitObj.attackDamage),
        range: unitObj.range,
