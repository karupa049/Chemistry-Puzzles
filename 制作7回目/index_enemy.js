...
function updateGameLogic() {
    ...
    let currentInterval;
    if (enemiesSpawned < 5) {
        currentInterval = 4000; // 最初は4秒間隔（準備時間）
    } else if (enemiesSpawned < 15) {
        currentInterval = 2000; // 徐々に2秒間隔へ
    } else if (enemiesSpawned < 30) {
        currentInterval = 1200; // 中盤は1.2秒
    } else {
        currentInterval = 800;  // 終盤は0.8秒間隔のラッシュ
    }

    if (millis() - lastSpawnTime > currentInterval) {
        if (enemiesSpawned < TOTAL_ENEMIES_TO_SPAWN) {
            
            // 出現パターンの決定（周期的に種類を変える）
            // 基本: ノーマル
            // 5の倍数: タンク (硬い)
            // 3の倍数 (ただし5の倍数を除く): エラティック (変則)
    ...
    for (let i = enemies.length - 1; i >= 0; i--) {
        ...
        if (mainTower && mainTower.isAlive()) {
            ...
            if (distance < e.radius + mainTower.radius) {
                ...
                mainTower.takeDamage(1); // 基地へのダメージ
            }
        }
        ...
        if (e.isDead() || e.x < 0 || e.y < 0 || e.x > width || e.y > height) {
            if (e.isDead()) {
                ...
                if (e.type === 'tank') {
                    reward = enemyKillReward * 5; // Tankは5倍
                } else if (e.type === 'erratic') {
                    reward = enemyKillReward * 2; // Erraticは2倍
                } else {
                    reward = enemyKillReward / 2; // Normalは半分
                }
                // ▲▲▲ 修正ここまで ▲▲▲
...
class Enemy {
    ...
    constructor(x, y, path, type = 'normal') {
        ...
        if (this.type === 'tank') {
            ...
            // ★修正: 攻撃範囲を 160 に設定 (かなり遠くから撃ってきます)
    ...
    findTargetAndAttack(placedElementsArray, shockwavesArray) {
        ...
        for (let unit of placedElementsArray) {
            if (!unit.isDead()) { 
                ...
                // 指定された attackRange (120 or 160 or 70) を使用
