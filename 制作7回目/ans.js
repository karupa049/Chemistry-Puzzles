...
// ===================================
// ★追加: 逆引きガイド（マウスオーバーで足りない素材を教える）
// ===================================
function drawReverseRecipeGuide() {
    // 1. マウスの下にある配置済み元素を探す（未反応のものに限る）
    let target = null;
    for (let el of placedGameElements) {
    ...
    if (!target) return; // ターゲットがいなければ何もしない

    // 2. その元素を含むレシピをすべて検索し、足りないものを計算
    let guideList = [];

    for (let recipe of recipes) {
        // このレシピに、ターゲットの元素が含まれているか？
        ...
        // 周囲にある他の元素も考慮して、必要数を減らす
        // (すでに近くにHが1個あるなら、あと1個で済むようにする)
        let neighbors = placedGameElements.filter(el => 
            el !== target && 
            !el.reacted && 
        ...
        // 何か足りないものがあればリストに追加（完成済みなら反応ロジックが走るので無視）
        if (missingParts.length > 0) {
            guideList.push({
                name: recipe.name,
                missing: missingParts.join(', '), // "H×2, O×1" のような文字列
                color: recipe.color
            });
        }
    }
